...
function FloorPlan({ tables = [], selectedTableIds = [], onSelectTable, loading }) {
  ...
  return (
    <div className="rzv-card rzv-fm">
      ...
      <p className="rzv-fm__status">
        {loading
          ? "Checking live availability…"
          : !floorPlanReady
          ? "Pick a date, time and party size to see live table availability."
          : `${availableOnFloor} table(s) available on ${floor.label} for your selection.`}
      </p>
    </div>
  );
}
